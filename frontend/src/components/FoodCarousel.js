        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {items.slice(0, 4).map((item, i) => (
            <div key={i} onClick={() => onItemClick?.(item)} style={{ background: '#fffdf8', borderRadius: 12, overflow: 'hidden', cursor: 'pointer' }}>
              <div style={{ height: 70, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, background: 'linear-gradient(135deg,#ece4d6,#f5efe3)', overflow: 'hidden' }}>
                {(item.image_url || item.image) ? <img src={item.image_url || item.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : '🍽️'}
              </div>
      ...
      <div ref={ref} className="hide-scroll" style={{ display: 'flex', gap: 14, overflowX: 'auto', scrollBehavior: 'smooth', scrollSnapType: 'x mandatory' }}>
        {items.map((item, i) => (
          <div key={i} onClick={() => onItemClick?.(item)} style={{ flex: '0 0 200px', background: '#fffdf8', borderRadius: 12, overflow: 'hidden', cursor: 'pointer', scrollSnapAlign: 'center' }}>
            <div style={{ height: 120, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 40, background: 'linear-gradient(135deg,#ece4d6,#f5efe3)', overflow: 'hidden' }}>
              {(item.image_url || item.image) ? <img src={item.image_url || item.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : '🍽️'}
            </div>
